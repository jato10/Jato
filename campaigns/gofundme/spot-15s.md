# Global Beyond LLC — Spot GoFundMe 15s

Campaña: recaudación GoFundMe · $52,000
Empresa: Global Beyond LLC (e-commerce, Miami)
Cofundadores en cámara: Javier y Yudenis
Entrega: MP4 16:9, 15 s, audio en español latino

## Ficha técnica de generación

- Modelo: Kling v3.0 (`kling3_0`) — multi-shot + audio sync nativo
- Parámetros: `duration: 15`, `mode: pro`, `sound: on`, `aspect_ratio: 16:9`
- `start_image`: foto de referencia de Javier y Yudenis en el sofá (fija identidad y set)
- `end_image`: logo metálico GB sobre fondo negro/azul marino (tarjeta final)
- Costo estimado: 37.5 créditos por render

## Dirección de arte

Cinematográfico corporativo, cálido y confiable. Luz suave de estudio, poca
profundidad de campo. Paleta: negro, azul marino, plata metálica. Interior
moderno — sofá beige, consola azul oscuro, lámpara de pie, logo GB en la pared.
Push-in lento, cortes suaves. Música corporativa inspiradora por debajo de la voz.

## Guion con tiempos

| Tiempo | Plano | Voz |
|---|---|---|
| 0:00–0:03 | Plano medio de Javier a cámara | Javier: "Somos Javier y Yudenis, cofundadores de Global Beyond LLC en Miami." |
| 0:03–0:07 | Plano medio de Yudenis a cámara · texto en pantalla: **$52,000** | Yudenis: "Buscamos $52,000 para lanzar con inventario, logística y publicidad." |
| 0:07–0:09 | Primer plano de Javier, mirada directa | Javier: "Cada dólar se reporta." |
| 0:09–0:12 | Primer plano de Yudenis, sonrisa cálida · texto: **DONA O COMPARTE EL ENLACE** | Yudenis: "Dona o comparte el enlace. Gracias." |
| 0:12–0:15 | Tarjeta final: logo metálico GB, destello de luz sobre el metal, "Global Beyond LLC" | sin diálogo, música de cierre, fade out |

## Entrega

- Render: Kling v3.0 `pro`, 15 s, 1920x1080, `sound: on`, 16:9
- `start_image`: foto del sofá (fotograma inicial, fija rostros y set)
- `end_image`: logo GB metálico (tarjeta de cierre)
- Costo: 37.5 créditos
- MP4: `hf_20260828_172713_00100d52-279e-461f-b76a-42d5b342c4d7.mp4`

### Notas de revisión

- El modelo resolvió los cinco planos desde la descripción del prompt, no con
  cortes marcados por parámetro (`multi_shots: false`), así que los cortes
  pueden no caer exactamente en 0:03 / 0:07 / 0:09 / 0:12.
- Los textos en pantalla ($52,000 y DONA O COMPARTE EL ENLACE) se pidieron en
  el prompt. Si salen deformados, conviene sobreponerlos en post en vez de
  re-generar.
